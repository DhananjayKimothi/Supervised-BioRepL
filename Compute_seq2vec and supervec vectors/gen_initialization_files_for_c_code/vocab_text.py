...
def txt_prnt(file, value, float_type = 0):
    ...
    for item in value:
        last += 1
        if float_type:
            item = format(item,'.5f') # it format the item explicitly to the float i.e 7.6325e-05, will formated as '0.00000763250' --type is str
        else:
            item = str(item)
        file.write(item)
        if last < len(value):
...
if __name__ == '__main__':
    ...
    if saveinit:
        for i in range(0,1):
            for j in range(0,1):
                savingtotxt(r"C:\Users\Dhananjay\Desktop\HSuperVec_BLAST\Largest_8class", file_no = j)
